...
unsigned long TS_CHANNEL_ID = 3389917;
const char * TS_WRITE_API_KEY = "DC1QHAZJFZXVINBN";
...
#define INTERVALO_MUESTRA        10000UL
#define INTERVALO_CLIMA          60000UL
#define INTERVALO_THINGSPEAK     30000UL
...
// ============================================================
// MÁQUINA DE ESTADOS — RIEGO AHORA ES BLOQUEANTE
// ============================================================
// Se ha removido la lógica de máquina de estados para hacer
// el riego 100% bloqueante según requerimientos de demo.
...
unsigned long ultimoThingSpeak = 0;
...
void  iniciarRiego(bool esManual);
void  gestionarRiego();
void  moverServos();
void  detenerServos();
void  actualizarOLED();
...
void setup() {
  ...
  delay(200);
  Serial.println("============================================================");
  Serial.println(" SISTEMA DE RIEGO INTELIGENTE - ESP32 S3  v4");
  Serial.println("============================================================");
  ...
  digitalWrite(RELAY_PIN, HIGH);  // HIGH = relé apagado (lógica invertida)
  ...
  actualizarOLED();
  Serial.println("[SYS] Sistema listo  (riego no bloqueante + relé corregido)");
  Serial.println("============================================================");
}
...
void loop() {
  ...
  if (ahora - ultimaLectura >= INTERVALO_LECTURA) {
    ...
    // Indicador LED de bajo nivel de agua
    if (waterPct < 20) {
      setLED(true, false, false); // Rojo
    } else {
      setLED(false, false, true); // Azul (estado normal)
    }
  }

  // =========================================================
  // BOTÓN MANUAL — dispara el riego (bloqueante)
  // =========================================================

  if (digitalRead(BUTTON_PIN) == LOW &&
      (ahora - ultimoBoton) > DEBOUNCE_MS) {
    ultimoBoton = ahora;
    iniciarRiego(true);   // true = manual
  }

  // =========================================================
  // RIEGO AUTOMATIZADO / MANUAL (Lógica bloqueante dentro de iniciarRiego)
  // =========================================================


  if (ahora - ultimoClima >= INTERVALO_CLIMA) {
    ultimoClima = ahora;
    obtenerClima();
  }

  if (ahora - ultimoThingSpeak >= INTERVALO_THINGSPEAK) {
    ultimoThingSpeak = ahora;
    enviarThingSpeak();
  }
...
void tomarMuestra() {
  struct tm ti;
  int hora = 0;
  if (getLocalTime(&ti, 10)) hora = ti.tm_hour;
...
void ejecutarIA() {
  ...
  if (output >= 0.5f) {
    decisionIA = "REGAR";
    iniciarRiego(false);   // false = automático por IA
...
// ============================================================
// INICIAR RIEGO — BLOQUEANTE (Pausa todo el código)
// ============================================================

void iniciarRiego(bool esManual) {
  // Si es IA y no hay agua, abortar. Si es manual, forzar el riego siempre.
  if (!esManual && waterPct < 10) {
    decisionIA = "Sin agua";
    Serial.println("[RIEGO] Abortado: nivel bajo (IA)");
    return;
  }

  regandoManual = esManual;
  regandoAuto   = !esManual;
  if (esManual) {
    decisionIA = "Riego Manual";
    Serial.println("[BOTON] Click detectado! Iniciando riego forzado...");
  }

  Serial.println("[RIEGO] Iniciando riego bloqueante...");
  digitalWrite(RELAY_PIN, LOW);   // LOW = relé encendido (lógica invertida)
  setLED(false, true, false);

  // 2 recorridos completos de 0 a 180 y regreso
  for (int i = 0; i < 2; i++) {
    // 0 a 180
    for (int angulo = 0; angulo <= 180; angulo += 10) {
      servo1.write(angulo);
      servo2.write(angulo);
      delay(500); // 10 en 10 cada medio segundo
    }
    // 180 a 0
...
// Funciones vacías para evitar errores de compilación con prototipos
void gestionarRiego() {}
void moverServos() {}
void detenerServos() {}
...
void actualizarOLED() {
  ...
  if (pantallaActual == 0) {
    ...
    if (waterPct < 20) {
      display.setCursor(0,34); display.println("Agua: Nivel Bajo");
    } else {
...
void enviarThingSpeak() {
  ...
  ThingSpeak.setField(1, porcH1);
  ThingSpeak.setField(2, porcH2);
  ThingSpeak.setField(3, waterPct);
  ThingSpeak.setField(4, tempDHT);
  ThingSpeak.setField(5, humDHT);
  ThingSpeak.setField(6, confianzaIA);
  ThingSpeak.setField(7, vecesRegado);
